   ...
   <link crossorigin="" href="https://fonts.gstatic.com/" rel="preconnect" />
    ...
    <style type="text/tailwindcss">
      ...
      :root {{
        ...
        --shadow-strong:  0 6px 18px rgba(2,10,22,.08);
      }}
    }}
    ...
    @layer utilities {{
      ...
      .panel       {{ background: var(--bg-panel); border: 1px solid color-mix(in srgb, var(--bd-strong) 82%, transparent); box-shadow: none; }}
      .btn-brand   {{ background: var(--brand); color: white; }}
      .btn-brand:hover {{ background: var(--brand-600); box-shadow: none; }}
      .text-white {{ color: var(--fg-primary) !important; }}
      .text-gray-400 {{ color: var(--fg-muted) !important; }}
      .text-gray-300 {{ color: var(--fg-secondary) !important; }}
      .text-gray-200 {{ color: var(--fg-secondary) !important; }}
    }}
...
<body>
    <div class="relative flex size-full min-h-screen flex-col group/design-root overflow-x-hidden"
        style='font-family: "Spline Sans", "Noto Sans", sans-serif; background: #f5f7fb;'>
        <div class="layout-container flex h-full grow flex-col">

            <!-- Header -->
            <header
                class="flex items-center justify-between whitespace-nowrap border-b border-solid border-b-[var(--bd-muted)] px-5 sm:px-10 py-3 sm:py-4 fixed top-0 left-0 right-0 bg-white z-50 font-['Space_Grotesk',_'Noto_Sans',_sans-serif]">
                <div class="flex items-center gap-3 text-white">
                    <svg class="text-[var(--brand)]" fill="none" height="24" stroke="currentColor" stroke-linecap="round"
                    ...
                    <div>
                        <h2 class="text-white text-lg sm:text-xl font-bold leading-tight tracking-[-0.015em]">
                            <a href="../index.html" class="hover:text-[var(--brand)] transition-colors">
            ...
            <main class="flex-1 mt-20 pb-16">
                <!-- Hero Section -->
                <section class="px-4 sm:px-8 lg:px-20 py-10 sm:py-12" style="background: var(--bg-surface);">
                    <div class="max-w-4xl mx-auto">
                        <!-- Back link -->
                        <a href="../projects.html"
                            class="inline-flex items-center gap-2 text-[var(--fg-muted)] hover:text-[var(--brand)] transition-colors mb-6">
...
{tags}
                ...
                <!-- Content Section -->
                <section class="px-4 sm:px-8 lg:px-20 py-10">
                    <div class="max-w-4xl mx-auto">

                        <!-- Project Media -->
                        <div class="panel rounded-2xl p-3 mb-6 reveal">
{media}
                        </div>

                        <!-- Project Description -->
                        <article class="prose prose-invert max-w-none reveal">
                            <h2 class="text-white text-xl sm:text-2xl font-bold mb-5">Overview</h2>
{description}

                            <!-- External Links -->
                            <div class="mt-6">
                                <h3 class="text-white text-lg font-bold mb-3">Resources</h3>
            ...
            <footer class="border-t border-solid border-t-[var(--bd-muted)] px-5 sm:px-10 py-4 z-50"
                ...
                <div class="max-w-7xl mx-auto flex justify-between items-center text-white">
                    <div class="flex items-center gap-3">
                        <p class="text-[var(--fg-secondary)] text-sm"> 2026<span class="text-subtle mr-2 ml-2">|</span>
...
def extract_card_data(card_html):
    ...
    list_match = re.search(r'<div class="text-\[#66758a\][^>]*>(.*?)</div>', card_html, re.DOTALL)
    if list_match and ('<ul' in list_match.group(1) or '<li' in list_match.group(1)):
        ...
        list_content = list_content.replace('text-[#66758a]', 'text-[var(--fg-secondary)]')
        desc_html = f'                            <div class="text-[var(--fg-secondary)] text-lg leading-relaxed mb-6">\n                                {list_content}\n                            </div>'
    else:
        # Try paragraph-based description
        desc_match = re.search(r'<p class="text-\[#66758a\][^>]*>(.*?)</p>', card_html, re.DOTALL)
        if desc_match:
    ...
    # Extract tags
    tags = re.findall(r'<span class="px-[12] py-1 [^"]*text-xs[^"]*rounded-full[^"]*">(.*?)</span>', card_html)
    tags_html = '\n'.join([f'                            <span class="px-3 py-1 rounded-full text-sm bg-[var(--brand)]/15 text-[var(--brand)]">{tag}</span>' for tag in tags])
...
PROJECT_MAP = {
    'AI/Score-BasedGenerativeModeling.html': 'score_based_generative_modeling.html',
